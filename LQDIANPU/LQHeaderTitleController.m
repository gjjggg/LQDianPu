...
#import "LQHeaderTitleController.h"



@interface LQHeaderTitleController ()<UIScrollViewDelegate>
...
///记录headerView控制LQHeaderViewStyle的约束 样式改变时移除约束
...
@implementation LQHeaderTitleController
...
- (void)setHeaderView:(UIView *)headerView
{
    ...
    if (self.headerViewStyle == LQHeaderViewStyleCenter) {
        //水平垂直居中
...
- (void)setHeaderViewStyle:(LQHeaderViewStyle)headerViewStyle
{
    if (!_headerView) { //没有头部视图
        ...
        if (_headerViewStyle != headerViewStyle) {
            ...
            if (headerViewStyle == LQHeaderViewStyleCenter) {
                //水平垂直居中
                [self topViewAddCenterContraints];
            } else {
                //底部对齐
